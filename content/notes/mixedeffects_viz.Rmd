...
date: 2015-04-10
tags: visualization, lattice, model checking, statistical graphics, GLMM
modified: 2015-04-26
output:
    md_document:
...
```
In a live walkthrough on April 10 at the Davis R-Users Group, I gave a brief presentation
...
the experimentals design and statistics behind modern mixed models, I recommend
...
implies the main problems occured when fitting a model that gets
the hierarchy wrong (e.g., a GLM to data generated from a hierarchical
process underperforms relative to a tranform + LMM)
...
advisor that speaks only ANOVA, as is often the case for workers in crop and
...
also help with design and interpretation, but are primariliy hands-on.  The
...
* using the native plotting capabilites (to the extent possible)

**three facets** of visualization

* quick diagnostic (plots *native* to fitting packages)
* slower model criticism (also *native*)
* inference & prediction (generally lightweight external libs / export capabilities)
...
plotting capabilites designed just for this purpose. Here's a demo
with the full (4-way interacting) model

```{r use-lsmeans-mean-plots}
## look at consequences
if(require(lsmeans)){
    ## here for 4-way interaction model (m2) -- mostly to see how can quicky plot response
...
## if residuals looking ok -- fastest way to get a quick look at coeffiiencent uncertainty
...
profile, and boostrap. For the random effects, the output of bootstrap and
...
##   strong effect ot nutrient homogenetity
...
Method `ggplot2::fortify` adds fitted and resid to data frame. Newish cran
...
## (as assed in first plot above -- second is the sqrt
## we could later drop theese outlier and see if they cha
...
Because the fixed effect paramter on size difference and the random effect
...
* for more meaninful plots, we should label the plot y-axis with unscaled
...
of `predict.merMod` in `lme4`. Here I look at predictions accounting for alli,
...
the predictions accoung just for random effects at the level of individual
females (blue dots), and the raw data.


```{r ugly-plot, eval = FALSE}
## could plot them all together -- code below gives some insight ## intot the
